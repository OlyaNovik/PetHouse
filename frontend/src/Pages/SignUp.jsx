import s from './Style_Page/SignUp.module.scss';
...
const SignUp = () => {
    ...
    function isValidConfirm(password, confpass) {
    ...
    const errorState = {
        ...
        firstNameError: 'none',
        lastNameError: 'none',
        confirmpassError: 'none',

    }
    const [formData, setFormData] = useState(initialFormState);
    const [error, setError] = useState(errorState)
    const [btnvision, Setbtn] = useState(true)
    ...
    const signUpHandler = async () => {
        const allValuesAreNone = Object.values(error).every(value => value === 'none');
    ...
    const validateField = (text, type) => {
        if (type === 'firstname') {
            if (isValidName(text) === false) {
                setError({ ...error, firstNameError: 'block' })
            } else setError({ ...error, firstNameError: 'none' })
        }
        if (type === 'lastname') {
            if (isValidName(text) === false) {
                setError({ ...error, lastNameError: 'block' })
            } else setError({ ...error, lastNameError: 'none' })
        }
        if (type === 'email') {
            if (isValidEmail(text) === false) {
                setError({ ...error, emailError: 'block' })
            } else setError({ ...error, emailError: 'none' })
        }
        if (type === 'password') {
            if (isValidPassword(text,formData.confpass) === false) {
                setError({ ...error, passwordError: 'block' })
            } else setError({ ...error, passwordError: 'none' })
        }
        if (type === 'confirmpass') {
            if (isValidConfirm(formData.password, text) === false) {
                setError({ ...error, confirmpassError: 'block' })
            } else setError({ ...error, confirmpassError: 'none' })
        }
    };
    // const DisableBtn = (e) => {
    //     const allValuesAreNone = Object.values(error).every(value => value === 'none');
    ...
    return (
        <>
            <div className={s.main_logo}>
                <div className={s.form_signup}>
                    <NavLink to="/#"><div className={s.cont_logo}>
                        {/* <img src={Logo} className={s.logo} alt="" /> */}
                    </div></NavLink>
                    <div className={s.main_text}><p>Register now!</p></div>
                    <div className={s.signup}>
                        <form action="" className={s.inp_group}>
                            <div className={s.group_err}>   <label htmlFor="name">Enter your first name</label>
                                <label htmlFor="name" id={s.error} style={{ display: error.firstNameError }} >*entered incorrectly</label></div>
                            <input
                                type="text"
                                id={s.name}
                                name='name'
                            ...
                            />
                            {/* <p id={s.error} style={{ display: error.firstNameError }} >*entered incorrectly</p> */}
                            <div className={s.group_err}>   <label htmlFor="name">Enter your last name</label>
                                <label htmlFor="name" id={s.error} style={{ display: error.lastNameError }} >*entered incorrectly</label></div>
                            <input
                                type="text"
                                id={s.last_name}
                                name='last_name'
                            ...
                            {/* <p id={s.error} style={{ display: error.lastNameError }} >*entered incorrectly</p> */}

                            <div className={s.group_err}><label htmlFor="email">Enter your email address</label>
                                <label htmlFor="name" id={s.error} style={{ display: error.emailError }} >*entered incorrectly</label>
                            </div>
                            <input
                                type="text"
                                id={s.email}
                            ...
                            {/* <p id={s.error} style={{ display: error.emailError }} >*entered incorrectly</p> */}

                            <div className={s.group_err}>   <label htmlFor="password">Password</label>
                                <label htmlFor="name" id={s.error} style={{ display: error.passwordError }} >*entered incorrectly</label>
                            </div>
                            <input
                                type={inputType}
                                name="password"
                            ...
                            <div className={s.group_err}>   <label htmlFor="password">Confirm password</label>
                                <label htmlFor="name" id={s.error} style={{ display: error.confirmpassError }} >*entered incorrectly</label>
                            </div>
                            <input
                                type={inputTypeConf}
                                name="password"
                    ...
                    <div className={s.footer_sign}>© 2023 PetHome. All rights reserved.</div>
