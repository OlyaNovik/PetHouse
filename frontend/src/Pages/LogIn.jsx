import s from './Style_Page/LogIn.module.scss';
import { useEffect, useState } from 'react'
import { NavLink, useNavigate } from 'react-router-dom'

import { connect } from 'react-redux'
import { signInHandler } from '../auth/actions/userActions';
...
const LogIn = ({ signInHandler }) => {
  ...
  const errorState = {
    emailError: 'none',
    emailMessage: '*entered incorrectly',
    passwordError: 'none',
    passMessage: '*entered incorrectly'
  }
  // const [errorEmail, SetErrorEmail] = useState('*entered incorrectly')
          ...
          <div className={s.formLogIn}>
            <form action="" className={s.inp_group}>
              <div className={s.group_err}><label htmlFor="email">Enter your email address</label>
                <label htmlFor="name" id={s.error} style={{ display: error.emailError }} >{error.emailMessage}</label>
              </div>
              <input
                type="text"
                id={s.email}
              ...
              <div className={s.group_err}>   <label htmlFor="password">Password</label>
                <label htmlFor="name" id={s.error} style={{ display: error.passwordError }} >{error.passMessage}</label>
              </div>
              <input
                type={inputType}
                name="password"
              ...
              />
              <div className={s.eye_cont} onClick={Vision}>
                {vision_pass}
              </div>
            </form>
            <div className={s.reset_pass}>
              <NavLink to="resetpassword">Forgot password?</NavLink>
            </div>
            <div className={s.btn_log}>
              <button onClick={() => { signInHandler(formData,error,setError, navigate) }}>Log In</button>

            </div>
          </div>
          <div className={s.footer_log}>
            <p>© 2023 PetHome. All rights reserved.</p>
          </div>
        </div>
      </div>
    </>
...
export default connect(null, { signInHandler })(LogIn);